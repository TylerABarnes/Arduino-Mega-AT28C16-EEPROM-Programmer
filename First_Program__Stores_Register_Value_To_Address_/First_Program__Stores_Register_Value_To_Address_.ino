...
//-----Last 5-bits are excluded --> Range: 0 - 2047
...
//WipeEEPROM();
FillEEPROM(0xea);
//Reverse bit order for the time being bc im an idiot
//Reads in big endian not little endian like it's supposed to
writeEEPROM(2044, 0x00);
writeEEPROM(2045, 0x01);
  //Tells CPU to go to address 0001
writeEEPROM(0, 0xA9); //Op code A9 - write value in next address to register A (LDA)
writeEEPROM(1, 0x42); //Loads into register A

writeEEPROM(2, 0x8D); //Stores value in A register to an address - decided in next two bytes (STA)
writeEEPROM(3, 0x01);
writeEEPROM(4, 0x00); //Stores at address 1000
